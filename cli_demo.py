...
def main():
    ...
    while not vs_path:
        ...
        filepath = "emotion"
        vs_path = os.path.join("/content/drive/My Drive", filepath, f"{filepath}_FAISS_20230714_165857", "vector_store")
        ...
        if not vs_path:
            for ext in LOADER_MAPPING:
                all_files.extend(
                    glob.glob(os.path.join(filepath, f"**/*{ext}"), recursive=True)
                )
        ...
        # 会直接导致TypeError: cannot unpack non-iterable NoneType object而使得程序直接退出
        # 因此需要先加一层判断，保证程序能继续运行
        temp, loaded_files = local_doc_qa.init_knowledge_vector_store(all_files, vs_path)
        if temp is not None:
            vs_path = temp
